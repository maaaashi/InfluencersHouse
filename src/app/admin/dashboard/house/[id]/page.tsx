...
export default function Page(props: Props) {
  ...
  const fetchHouse = async () => {
    const response = await fetch(`/api/houses/${props.params.id}`)
    const { house } = await response.json()
    setValue('name', house.name)
    setValue('description', house.description)
    setValue('place', house.place)
    setValue('event_date', house.event_date)
    setValue('owner_id', house.owner_id)
    setValue('invitations', house.invitations)
  }
  useEffect(() => {
    fetchOwner()
  ...
  return (
    <div className='flex flex-col justify-center py-8 px-4 container mx-auto gap-4'>
      <h2 className='font-bold self-start text-lg'>ハウス編集</h2>
      <form
